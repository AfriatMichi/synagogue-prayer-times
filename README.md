...
**למה לא cron תכוף?** `schedule` של GitHub הוא best-effort בלבד: הרצות נדחות
בעומס וחלקן נופלות לגמרי. בריפו הזה `*/5` לא רץ אף פעם במשך שעה שלמה, בעוד
שההגדרות היו תקינות. לכן הכיסוי נשען על אורך ההרצה ולא על תדירות התזמון.
`gh workflow run telegram-poller.yml` מפעיל מיד — והפעלה ידנית מבטלת הרצה פעילה
כדי שלא תמתין לסיום הלולאה.
