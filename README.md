...
הודעת WhatsApp מגבאי
   ...
   Telegram Bot
        ↓
GitHub Actions — 4 ניסיונות בשעה, ~10 דקות האזנה בכל הרצה
        ↓
...
**כמה זמן לוקח פרסום?** כל הרצה מאזינה ~10 דקות עם long polling של טלגרם,
ומנסה להתחיל ארבע פעמים בשעה. כשההרצה פעילה — ההודעה, התצוגה המקדימה והאישור
מטופלים תוך שניות, הכל באותה הרצה. אחרת ממתינים להרצה הבאה.

הלולאה קוצרה מ-50 דקות ל-10 בכוונה: בלולאה ארוכה שלושת הניסיונות הנוספים בשעה
נבלעים כ"ממתינים" מאחורי ההרצה הפעילה ולא נראים כלל, ואי אפשר לדעת כמה מהם
GitHub באמת מספק. המחיר הוא כיסוי — טריגר שעובר קונה 10 דקות של בוט חי ולא 50.

**התזמון של GitHub לא אמין — וזו לא תקלה אצלנו.** `schedule` הוא best-effort
בלבד: הרצות נדחות בעומס וחלקן נופלות בשקט, בלי שגיאה ובלי התראה. נמדד בריפו
הזה: **הרצה אחת מתוך שמונה צפויות, באיחור של 48 דקות.** ההגדרות תקינות
לחלוטין — ה־workflow `active`, Actions מאופשר, `main` הוא ברירת המחדל,
וה־cron נמצא בקובץ שעל `main`.

שני דברים נגזרים מזה, ושניהם מיושמים ב־`telegram-poller.yml`:

- **העומס מרוכז בראש השעה ובגבולות `*/5`.** לכן כל דקה ב־cron אינה כפולה של
  5, ואף אחת לא נמצאת בטווח חמש דקות מ־`:00`.
- **כל ניסיון בודד עלול להיעלם.** לכן ארבעה ניסיונות בשעה ולא אחד. ה־concurrency
  מגביל ל־run פעיל אחד ועוד אחד ממתין, אז אין הצטברות — ולהפך, ההרצה הממתינה
  מתחילה ברגע שהלולאה הנוכחית נגמרת ומשרשרת כיסוי לאורך השעה.

`gh workflow run telegram-poller.yml` מפעיל מיד, ומבטל הרצה פעילה כדי שלא תמתין
לסיום הלולאה.

**זה שיפור הסתברותי, לא תיקון.** הפתרון האמין היחיד הוא טריגר חיצוני
(למשל Cron Trigger של Cloudflare Worker שיורה `repository_dispatch`), או
העברת הבוט ל־webhook.
